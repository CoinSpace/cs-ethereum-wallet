...
class Wallet {
  ...
  createReplacement(tx) {
    const that = this;
    const gasPrice = Big(tx.gasPrice).times(that.replaceByFeeFactor).toFixed(0);
  ...
  sendTx(tx, done) {
    ...
    this.api.transactions.propagate(rawtx).then((txId) => {
      ...
      if (this.token) {
        ...
        this.processTx(tx, done);
      }
    }).catch(done);
  }
  processTx(tx, done) {
    this.api.transactions.get(tx.txId, this.addressString).then((historyTx) => {
      ...
      this.balance = Big(this.balance).plus(historyTx.amount).minus(historyTx.maxFee).toFixed(0);
